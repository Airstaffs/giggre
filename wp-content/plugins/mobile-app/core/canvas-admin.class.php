...
class CanvasAdmin {
	...
	public static $canvas_website_url  = CANVAS_WEBSITE_URL;
	public static $plugin_name         = 'mobiloud_canvas';
		...
		if ( get_transient( '__canvas_activation_redirect' ) && ( ! defined( 'DOING_AJAX' ) || ! DOING_AJAX ) ) {
			...
			wp_safe_redirect(
				add_query_arg(
					...
					admin_url( 'admin.php?page=canvas_app_builder' )
				)
			);
		}
	}
	...
	public static function on_admin_menu() {
		...
		self::$admin_screens[] = add_submenu_page( Canvas::$slug, 'Settings', 'Settings', 'activate_plugins', Canvas::$slug, array( 'CanvasAdmin', 'main_menu' ) );
	...
	public static function main_menu() {
		...
		// app builder experience start

		if (Canvas::get_option('app_builder_experience') === 'no') {
			if (isset($_GET['app-builder']) && $_GET['app-builder'] == 1 && is_admin()) {
				Canvas::set_option('app_builder_experience', 'yes');
				$data = base64_encode(json_encode([
					'w'  => base64_encode(site_url()),
					'pn' => base64_encode(self::$plugin_name)
				]));

				$utm_source = '/?utm_source=canvas_plugin&utm_medium=installation&utm_campaign=installation';
				$canvas_site = self::$canvas_website_url . '/?d=' . urlencode($data) . $utm_source;
				$mobiloud_demo_url = "https://www.mobiloud.com/demo" . $utm_source;
				echo "<script>window.location.href='" . $mobiloud_demo_url . "'</script>";
				exit();	
			}
			return CanvasViews::view('app-builder-welcome');
		}
		// app builder experience end
		...
		if ( count( $_POST ) && check_admin_referer( 'form-settings-' . $active_tab ) && ! isset( $_POST['configured'] ) ) {
			switch ( $active_tab ) {
				...
				case 'push':
					...
					// push notification tag start

					if ($_POST['push_tag_key'] && !empty($_POST['push_tag_key']) && $_POST['push_tag_value'] && !empty($_POST['push_tag_value'])) {
						$pushTagKeys = $_POST['push_tag_key'];
						$pushTagValues = $_POST['push_tag_value'];

						$tagData = [];
					
						for ($i = 0; $i < count($pushTagKeys); $i++) {
							if ($pushTagKeys[$i] && $pushTagValues[$i]) {
								$tagData[$pushTagKeys[$i]] = $pushTagValues[$i];
							}
						}
						Canvas::set_option('push_notification_tags', serialize($tagData));
					}
	...
	public static function save_theme() {
		check_ajax_referer( 'canvas_save_theme_nonce', 'security' );
		$use = isset( $_POST['use'] ) ? boolval( $_POST['use'] ) : false;
		$theme = isset( $_POST['theme'] ) ? sanitize_text_field( $_POST['theme'] ) : '';
		Canvas::set_theme( ! empty( $use ), $theme );
		die( 'Ok' );
	}
